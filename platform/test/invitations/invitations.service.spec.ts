...
function build() {
  ...
  // $transaction runs the callback with a dummy tx; repo methods are mocked
  // so the tx value itself is irrelevant to these unit tests.
  const prisma = { $transaction: jest.fn(async (fn: any) => fn({})) };
  ...
  return { svc, invitations, users, students, prisma };
}
...
describe('InvitationsService.issue', () => {
  it('admin can invite an instructor; returns the raw token once and persists only the hash', async () => {
    ...
    // Security: only the SHA-256 hash is persisted, never the raw token.
    const invArg = invitations.create.mock.calls[0][0];
    expect(invArg.tokenHash).toMatch(/^[0-9a-f]{64}$/);
    expect(invArg.tokenHash).not.toBe(res.token);
    expect(invArg).not.toHaveProperty('token');
  });

  it('instructor inviting forces role=student and links the student to themselves', async () => {
    const { svc, users, students, invitations } = build();
    users.findByEmail.mockResolvedValue(null);
    ...
    expect(invitations.create).toHaveBeenCalledWith(
      expect.objectContaining({ role: 'student', invitedById: 'inst-1' }),
      expect.anything(),
    );
  });
  ...
  it('rejects inviting an email that belongs to a disabled account', async () => {
    const { svc, users } = build();
    users.findByEmail.mockResolvedValue({ id: 'u', email: 'a@x.com', status: 'disabled' });
    await expect(svc.issue({ email: 'a@x.com', name: 'A', role: 'instructor' }, ADMIN))
      .rejects.toBeInstanceOf(BadRequestException);
  });
});
